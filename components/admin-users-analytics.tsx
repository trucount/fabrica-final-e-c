...
export function AdminAnalyticsPanel() {
  ...
  useEffect(() => {
    loadOrders().then(setOrders).catch((analyticsError) => setError(analyticsError instanceof Error ? analyticsError.message : "Analytics could not be loaded."))
  }, [])
  ...
  return (
    <section className="space-y-6 rounded-lg border p-4 sm:p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Umami tracking is installed site-wide, with commerce charts below for quick admin insight.</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-lg border bg-background p-4">
          ...
          {shareUrl ? <iframe title="Umami analytics" src={shareUrl} className="h-[260px] w-full rounded-md border" /> : <p className="flex h-[260px] items-center justify-center rounded-md bg-secondary p-6 text-center text-sm text-muted-foreground">Set NEXT_PUBLIC_UMAMI_SHARE_URL to embed your shared Umami dashboard here. The tracking script is already installed.</p>}
        </div>
      </div>
    </section>
  )
